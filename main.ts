...
import {SLIDEV_PRESENTATION_VIEW_TYPE, SlidevPresentationView} from "./views/SlidevPresentationView";
import {DEFAULT_SETTINGS, SlidevPluginSettings, SlidevSettingTab} from "./SlidevSettingTab";
...
export default class SlidevPlugin extends Plugin {
	...
	async onload() {
		...
		const ribbonIconEl = this.addRibbonIcon('dice', 'Open Slidev Presentation View', (evt: MouseEvent) => {
			this.activateView();
		});
	...
	async activateView() {
		...
		this.app.workspace.revealLeaf(
			this.app.workspace.getLeavesOfType(SLIDEV_PRESENTATION_VIEW_TYPE)[0]
		);
	}
