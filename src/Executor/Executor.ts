...
export default abstract class Executor extends EventEmitter {
	...
	protected notifyError(
		...
		tempFileName: string,
		err: unknown,
		outputter: Outputter | undefined,
		label = "Error while executing code",
	) {
		const errorMSG = `Error while executing ${cmd} ${cmdArgs} ${tempFileName}: ${String(err)}`;
	...
	protected getTempFile(ext: string) {
		...
		return `${tmpdir()}/temp_${this.tempFileId}.${ext}`;
	}
}
